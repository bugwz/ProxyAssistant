if (typeof importScripts === 'function') importScripts('rule-matcher.js');
...
function setProxyAuthentication(proxies) {
  activeAuthProxies = proxies.filter(proxy => proxy && proxy.enabled !== false && proxy.ip && proxy.port)
    .map(proxy => ({
      ip: String(proxy.ip).toLowerCase(), port: Number(proxy.port),
      username: proxy.username || '', password: proxy.password || ''
    }));
  if (chrome.storage.session) {
    chrome.storage.session.set({ proxy_auth: activeAuthProxies }, () => {
      if (chrome.runtime.lastError) console.info('Failed to persist proxy authentication:', chrome.runtime.lastError.message);
    });
  }
}

function getScenarioProxies(config) {
  return config?.scenarios?.lists?.find(scenario => scenario.id === config.scenarios.current)?.proxies || [];
}
...
function restoreProxySettingsNow() {
  ...
  return storagePromise.then(async ({ local: result, session: sessionResult }) => {
    if (isFirefox) {
      ...
      firefoxProxyState.currentProxy = resolveStoredManualProxy(result.config, result.state?.proxy?.current);
      // If enabled, restore mode
      if (result.state?.proxy?.mode && result.state.proxy.mode !== 'disabled') {
        firefoxProxyState.mode = result.state.proxy.mode === 'manual' && !firefoxProxyState.currentProxy
          ? 'disabled' : result.state.proxy.mode;
      } else {
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === 'local') {
    ...
    if (isFirefox && changes.config) {
      ...
      if (firefoxProxyState.mode === 'auto') setProxyAuthentication(getScenarioProxies(currentConfig));
    }
...
function resolveStoredManualProxy(config, savedProxy) {
  const scenario = config?.scenarios?.lists?.find(item => item.id === config.scenarios.current);
  return scenario?.proxies?.find(proxy => (
    proxy.enabled !== false && proxy.ip && proxy.port &&
    (savedProxy?.id ? proxy.id === savedProxy.id :
      savedProxy && proxy.ip === savedProxy.ip && String(proxy.port) === String(savedProxy.port))
  )) || null;
}
...
function isIpPattern(...args) {
  return ProxyRuleMatcher.isIpPattern(...args);
}

function isSafeProxyRegexSource(...args) {
  return ProxyRuleMatcher.isSafeProxyRegexSource(...args);
}

function parseProxyRegexPattern(...args) {
  return ProxyRuleMatcher.parseProxyRegexPattern(...args);
}

function splitProxyRulePatterns(...args) {
  return ProxyRuleMatcher.splitProxyRulePatterns(...args);
}
...
function ipToNumber(...args) {
  return ProxyRuleMatcher.ipToNumber(...args);
}

function isInCidrRange(...args) {
  return ProxyRuleMatcher.isInCidrRange(...args);
}
...
function isIPv4Address(...args) {
  return ProxyRuleMatcher.isIPv4Address(...args);
}

function compileFirefoxRulePatterns(...args) {
  return ProxyRuleMatcher.compileFirefoxRulePatterns(...args);
}
...
function matchesCompiledFirefoxRules(...args) {
  return ProxyRuleMatcher.matchesCompiledFirefoxRules(...args);
}
...
// Only offer credentials belonging to the proxy that issued the challenge.
async function handleAuthRequest(details, callback) {
  if (!details.isProxy || !details.challenger) {
    callback({ cancel: false });
    return;
  }
  try {
    if (activeAuthProxies === null) {
      const session = chrome.storage.session
        ? await callStorageArea(chrome.storage.session, 'get', 'proxy_auth') : {};
      if (activeAuthProxies === null) activeAuthProxies = session?.proxy_auth || [];
    }
    const candidates = activeAuthProxies.filter(proxy => (
      proxy.ip === String(details.challenger.host).toLowerCase() &&
      proxy.port === Number(details.challenger.port) && proxy.username
    ));
    const proxy = candidates[0];
    const unambiguous = proxy && candidates.every(candidate => (
      candidate.username === proxy.username && candidate.password === proxy.password
    ));
    callback(unambiguous ? { authCredentials: { username: proxy.username, password: proxy.password } } : { cancel: false });
  } catch (error) {
    console.info('Failed to resolve proxy authentication:', error);
    callback({ cancel: false });
  }
}
