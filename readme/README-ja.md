...
| ストレージタイプ | 説明 |
|-----------------|------|
| **ローカルストレージ (local)** | 常に有効、プロキシリストとすべての構成データを保存し、オフライン利用を確保 |
| **クラウド同期 (sync)** | オプション、ブラウザアカウントを介して同一ブラウザの複数デバイス間で同期 |
...
### 7.2 シナリオ2: スマートルーティング

- 国内ウェブサイトは直接接続
- 特定のウェブサイトはプロキシ経由
- ドメインに基づいて自動選択

### 7.3 シナリオ3: プロキシポールテスト
