...
### 刷新按钮交互约束
- 项目中所有用户可见的刷新、重新获取和失败重试按钮都必须遵循同一反馈协议；新增刷新入口时应复用“当前配置”刷新按钮的交互，不得实现成无状态的普通点击按钮。
- 用户点击后必须同步将按钮设为 `disabled`，并在处理函数入口防止重复执行；请求完成、失败或抛出异常后必须恢复按钮状态，优先在 `finally` 或等价的统一收尾逻辑中处理。
- 刷新期间只旋转按钮内部的刷新图标，不得旋转整个按钮、替换按钮文字或让按钮尺寸发生变化；图标应使用稳定的尺寸和 `transform-origin: center`，动画速度统一为 `0.8s linear infinite`。
- 快速完成的刷新必须保留至少 `600ms` 的可感知反馈，避免图标一闪而过；等待时间必须按“`600ms - 已经过时间`”计算。实际请求已经超过 `600ms` 时应立即结束，不得在请求完成后再追加固定延迟。
- 页面初始化、切换页面、存储变更等非用户点击触发的自动刷新不得强制等待 `600ms`，避免拖慢首屏和后台同步；只有明确的用户点击需要最短反馈时间。
- 刷新内容区域存在加载态时，应同步维护 `aria-busy` 和加载元素的可见状态，并在必要时锁定原高度，避免刷新期间布局跳动；无内容加载态的紧凑刷新按钮至少要保留禁用态和内部图标动画。
- 亮色、暗色主题中的刷新按钮在刷新期间都必须使用 `opacity: 0.65` 的禁用视觉，使按钮整体颜色变浅并与“当前配置”刷新按钮保持一致；该透明度规则只应用于刷新按钮，不得连带修改清空、删除等其他禁用按钮。在 `prefers-reduced-motion: reduce` 下必须关闭旋转动画，但仍保留 `opacity: 0.65` 和禁用状态作为非动画反馈。
- 刷新失败必须与成功使用相同的最短反馈和收尾流程，并显示项目现有的错误提示；不得因错误分支遗漏恢复按钮、`aria-busy`、内容高度或加载元素状态。
- 相关测试至少验证：连续点击只发起一次操作、刷新期间按钮被禁用且内部图标仍存在、`599ms` 时反馈未结束、`600ms` 时快速操作恢复、慢操作不追加延迟，以及成功和失败后状态均能恢复。
